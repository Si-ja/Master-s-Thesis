...
def see_layers(model,
    ...
    > view_range  = indicate if you want to see a shorter range than what the data tests on; DEFAULT: False and will
                   show all of the range;
    > range_value = if the view_range is True, then you can indicate how long of a range you'd like to see;
    --------------------------------------------------------------------------------------------------------------
    ...
    if view_range == True and range_values <= len(output):
        df = pd.DataFrame(output).transpose().iloc[:, : range_values]
    else:
    ...
    if visualize == True:
        ...
        plt.title("Units' Activations", fontsize=MEDIUM_SIZE)
        plt.xlabel("Prediction Instance", fontsize=MEDIUM_SIZE)
        plt.ylabel("Unit", fontsize=MEDIUM_SIZE)
...
def nn_analysis(model,
    ...
    plt.rc('xtick', labelsize = MEDIUM_SIZE)    
    plt.rc('ytick', labelsize = MEDIUM_SIZE)
    plt.title("K-means clustering of\nstandard deviations of units' outputs", fontsize=MEDIUM_SIZE)
    plt.xlabel("Criteria of distance\nbetween mean and min", fontsize=MEDIUM_SIZE)
    plt.ylabel("Criteria of distance\nbetween mean and max", fontsize=MEDIUM_SIZE)
    if visualize == True:
        plt.show()
    ...
    if save_fig == True:
        layers_plt = model.lstm.num_layers
        neurons_plt = model.lstm.hidden_size
        fname = str("K-means clustering of units using {} layer(s) and {} unit(s).pdf".format(layers_plt, neurons_plt))
        f.savefig(fname, bbox_inches='tight')
